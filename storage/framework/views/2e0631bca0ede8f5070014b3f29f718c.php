...
<?php $component->withAttributes([]); ?>
            ...
            <?php if(str_contains($alert['msg'], 'verifikasi email')): ?>
            <?php
            $resendEmail = '';
            if(old('username')) {
            $user = \App\Models\User::where('username', old('username'))->first();
            if($user) $resendEmail = $user->email;
            }
            ?>
            <form method="POST" action="<?php echo e(route('verification.resend')); ?>" class="mt-2">
              <?php echo csrf_field(); ?>
              <input type="hidden" name="email" value="<?php echo e($resendEmail); ?>">
              <button type="submit" class="btn btn-sm btn-warning mt-2"><?php echo e(__('Kirim Ulang Email Verifikasi')); ?></button>
            </form>
            <?php endif; ?>
            <?php $__env->endSlot(); ?>
...
<?php endif; ?><?php /**PATH D:\laragon\www\mpwa_v10\resources\themes/vuexy/views/auth/login.blade.php ENDPATH**/ ?>