    ...
    <div class="authentication-inner row m-0">
      ...
      <div class="d-flex col-12 col-xl-4 align-items-center authentication-bg p-sm-12 p-6">
        <div class="w-px-400 mx-auto mt-12 pt-5 position-relative">
          ...
          @php $alert = session('alert') ?? old('alert'); @endphp

          @if ($alert)
          <x-alert>
            @slot('type', $alert['type'])
            @slot('msg')
            {!! $alert['msg'] !!}
            @if (str_contains($alert['msg'], 'verifikasi email'))
            @php
            $resendEmail = '';
            if(old('username')) {
            ...
            <form method="POST" action="{{ route('verification.resend') }}" class="mt-2">
              ...
              <button type="submit" class="btn btn-sm btn-warning mt-2">{{ __('Kirim Ulang Email Verifikasi') }}</button>
            </form>
            @endif
            @endslot
          </x-alert>
